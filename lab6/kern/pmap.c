...
static u_long memsize; /* Maximum physical address 内存大小 */
u_long npage;	       /* Amount of memory(in pages) 总物理内存页数*/

Pde *cur_pgdir; /* 当前页目录 */

struct Page *pages; /* 物理页面 */
static u_long freemem; /* 空闲内存地址 */

struct Page_list page_free_list; /* Free list of physical pages 空闲内存列表 */
...
void *alloc(u_int n, u_int align, int clear) {
	 ...
	 * linker did *not* assign to any kernel code or global variables. */
	 //这里是虚拟地址，但由于这个虚拟地址位于kesg0，可以直接操控物理内存
	if (freemem == 0) {
	...
	// Panic if we're out of memory.      panic_on是输出错误信息的
 ...
 */
//给定一个虚拟地址，在给定的页目录中查找这个虚拟地址对应的页表项
...
int page_insert(Pde *pgdir, u_int asid, struct Page *pp, u_long va, u_int perm) {
	Pte *pte;
	// 由于需要将 pp 对应页映射到 va，首先检查是否 va 已经存在了映射？假如有，则是pte
