...
void hist_init()
{
	...
	if ((fd = create("/.history", FTYPE_REG)) < 0)
	{
		user_panic("Fail to initialize .history: %d.", fd);
	}
}

void hist_store(char *s)
{
	int fd;
	if (strlen(s) == 0)
	{
	...
	if ((fd = open("/.history", O_RDWR | O_CREAT | O_APPEND)) < 0)
	{
...
int get_hist(char (*history)[1024])
{
	char buf[8192];
	memset(buf, 0, 8192);
	int fd;
