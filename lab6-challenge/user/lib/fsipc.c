...
//  < 0 on failure.
/*
int fsipc_map(u_int fileid, u_int offset, void *dstva): 该函数用于向文件服务器发送映射块请求。函数首先定义一个struct Fsreq_map类型的请求结构体指针req，并将文件的fileid和偏移量offset设置到请求结构体中。
然后调用fsipc函数，将请求类型设置为FSREQ_MAP，请求结构体指针设置为req，目标虚拟地址（destination virtual address）设置为dstva，并传递一个指针用于接收权限信息。
如果fsipc返回值小于0，则直接返回该错误码。

在请求发送成功并得到响应后，函数会检查返回的权限信息。
如果权限信息不符合预期（不是仅包含PTE_V标志且不包含其他标志），则会触发一个用户级异常（user_panic），打印出权限信息和目标虚拟地址，并终止程序运行。

如果一切顺利，函数返回0表示映射请求成功。
*/
