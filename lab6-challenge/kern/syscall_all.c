...
int sys_write_dev(u_int va, u_int pa, u_int len)
{
	...
	// pa
	if ((pa >= DEV_CONS_ADDRESS && pa + len <= DEV_CONS_ADDRESS + DEV_CONS_LENGTH) || (pa >= DEV_DISK_ADDRESS && pa + len <= DEV_DISK_ADDRESS + DEV_DISK_BUFFER + DEV_DISK_BUFFER_LEN) || (pa >= DEV_RTC_ADDRESS && pa + len <= DEV_RTC_ADDRESS + DEV_RTC_LENGTH))
	{
		u_int kva = pa + 0xa0000000;
		memcpy((void *)kva, (void *)va, len);
		return 0;
	}
	return -E_INVAL;
}
...
int sys_read_dev(u_int va, u_int pa, u_int len)
{
	...
	if ((pa >= DEV_CONS_ADDRESS && pa + len <= DEV_CONS_ADDRESS + DEV_CONS_LENGTH) || (pa >= DEV_DISK_ADDRESS && pa + len <= DEV_DISK_ADDRESS + DEV_DISK_BUFFER + DEV_DISK_BUFFER_LEN) || (pa >= DEV_RTC_ADDRESS && pa + len <= DEV_RTC_ADDRESS + DEV_RTC_LENGTH))
	{
		u_int kva = pa + 0xa0000000;
		// kva复制到va
		memcpy((void *)va, (void *)kva, len);
		return 0;
	}
	return -E_INVAL;
}
static int shellIdNow = 0;
static char names[1024][64];
static char values[1024][256];
static int is_readonly[1024];
static int var_shell_id[1024];
...
{
	// printk("envar 1 times\n");
	int pos = 0;
	// 寻找当前名字
	while (names[pos][0])
	{
		if (strcmp(names[pos], name) == 0)
		{
			break;
		}
		else
	...
	if (op == VAR_Create)
	{
		if (strcmp(names[pos], name) == 0 && var_shell_id[pos] != -2) // 名字同且没被删除
		{
			printk("envar %s has existed\n", name);
		...
		{
			var_shell_id[pos] = shellIdNow;
		}
		// printk("succes create name = %s value = %s\n", names[pos], values[pos]);
	}
	else if (op == VAR_GetOne)
	{
		if ((strcmp(names[pos], name) == 0) && (var_shell_id[pos] == -1 || var_shell_id[pos] == shellIdNow))
		{
			strcpy(value, values[pos]);
		}
		else
		{
			printk("Current shell doesn't contain this variable!\n");
			return -E_NOT_FOUND;
		}
	}
	else if (op == VAR_SetValue)
	{
		if (!(var_shell_id[pos] == -1 || var_shell_id[pos] == shellIdNow))
		{
			printk("Current shell doesn't contain this variable!\n");
			return -E_NOT_FOUND;
		}
		else if (is_readonly[pos])
		{
			printk("This variable can't be changed because it's read only!\n");
			return -E_NOT_EXEC;
		}
		else
		{
	...
	else if (op == VAR_GetAll)
	{
		for (int i = 0; i < 1024; i++)
		{
			if (names[i][0] != 0 && (var_shell_id[i] == -1 || var_shell_id[i] == shellIdNow))
			{
				printk("Name: %-20sValue: %-20s", names[i], values[i]);
				if (var_shell_id[i] == -1)
				{
					printk(" --[ENVIRONMENT VARIABLE]");
				}
				else
				{
					printk(" --[LOCAL VARIABLE]");
				}
				if (is_readonly[i])
				{
					printk(" --[READ_ONLY]");
