 ...
 *
 */
 //将参数指定的程序段（program segment）加载到进程的地址空间中。
 ...
 *   segments.
 */
 //负责加载可执行文件 binary 到进程 e 的内存中
static void load_icode(struct Env *e, const void *binary, size_t size) {
	/* Step 1: Use 'elf_from' to parse an ELF header from 'binary'.解析 ELF 文件头的部分 */
	...
	ELF_FOREACH_PHDR_OFF (ph_off, ehdr) {
		...
		if (ph->p_type == PT_LOAD) {
			...
			// should be mapped.
			//elf_load_seg 负责将 ELF 文件的一个 segment 加载到内存
