...
/*
 * 创建一个名称为 name 链表的头部结构体，包含一个指向 type类型结构体的指针，这个指针可以指向链表的首个元素
 ...
 * x as the list piece.
 *
 * 本质是一个链表项，包括指向下一个元素的指针 le_next，以及指向前一个元素链表项 le_ne    xt 的指针 le_prev。le_prev 是一个指针的指针，它的作用是当删除一个元素时，更改前一个元
    素链表项的 le_next。
 ...
 * 判断链表 head（头部结构体 head 对应的链表）是否为空
 */
#define LIST_EMPTY(head) ((head)->lh_first == NULL)

/*
 * 将返回链表 head 对应的链表的首个元素。
...
/* 
 *返回 elm 在对应链表中指向的下一个元素。elm 结构体需要包含一个名为 field 的字段，类型是一个链表项 LIST_ENTRY(type)，下面出现的 field 含义均和此相同。
